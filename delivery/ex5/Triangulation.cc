...
Triangulation::Vertex* Triangulation::updateFace(Face* face, const Vector3& p)
{
    ...
    if (is_reference_face)
    {
        if (isPointInFace(face, aux_point))
        {
            aux_point_face = face;
            is_reference_face = false;
        }
        else if (isPointInFace(face2, aux_point))
        {
            aux_point_face = face2;
            is_reference_face = false;
        }
        else if (isPointInFace(face3, aux_point))
        {
            aux_point_face = face3;
            is_reference_face = false;
        }
    }
    return new_vertex;
